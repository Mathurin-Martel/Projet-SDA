...
- `CH.java` : algorithme Contraction Hierarchies — regroupe le prétraitement (contraction + orientation du graphe) et la requête (Dijkstra bidirectionnel sur le graphe CH).
...
Le programme charge le graphe, lance le prétraitement ALT et CH, lance les algorithmes sur une requête de démonstration, puis exécute le benchmark.

## Algorithme CH (Contraction Hierarchies)

L'algorithme CH se déroule en deux phases :

**Prétraitement** (dans `CH.java`, méthode `pretraitement`) :
1. Lecture du graphe brut (`donnees_osmnx.csv`).
2. Attribution d'un rang à chaque nœud selon son degré.
3. Contraction : pour chaque nœud (ordre croissant de rang), ajout de raccourcis entre ses voisins si nécessaire pour préserver les plus courts chemins.
4. Orientation : ne conserver que les arêtes allant vers les nœuds de rang supérieur.

**Requête** (méthode `executer`) :
- Dijkstra bidirectionnel sur le graphe orienté CH.
- La recherche avant part du départ, la recherche arrière part de l'arrivée.
- Le meilleur nœud de jonction donne la distance optimale.
